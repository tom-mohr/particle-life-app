...
// InputFloat Block
{
    ...
    if (ImGui.inputFloat("rmax##Input", rmaxInputValue, 0.005f, 1.000f, "%.3f", ImGuiInputTextFlags.EnterReturnsTrue)) {
        ...
        loop.enqueue(() -> physics.settings.rmax = newRmax);
                    ...
                    {
                    ImFloat VelocityInputValue = new ImFloat((float) settings.velocityHalfLife);
                        if (ImGui.inputFloat("Velocity##Input", VelocityInputValue, 0.005f, 1.000f, "%.3f", ImGuiInputTextFlags.EnterReturnsTrue)) {
                        final double newVelocityFactor = Math.max(0.005f, Math.min(VelocityInputValue.get(), 1.000f)); // Clamping the value within a range
                        loop.enqueue(() -> physics.settings.velocityHalfLife = newVelocityFactor);
                        }
                    }
